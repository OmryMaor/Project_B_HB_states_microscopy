...
for i, eta in enumerate(etas_to_test):
    if valid_N[eta]:
        ...
        noisy_vals = [noisy_noon(N, d_fixed, eta) for N in valid_N[eta]]
        ax.plot(valid_N[eta], noisy_vals, '--', color=colors[i], alpha=0.7,
                label=r'N00N $\eta=%s$' % eta)

# Ideal limits (Dashed/Dotted)
ax.plot(N_values_a, multipixel_a, ':', color='red', lw=1.5,
        label=r'Optimal $|\psi_s\rangle$ (Ideal)')
ax.plot(N_values_a, noon_a, '--', color='green', lw=1.5,
...
for i, eta in enumerate(etas_to_test):
    if valid_N[eta]:
        ...
        # Noisy N00N (Dashed, same color)
        noisy_vals_zoom = [noisy_noon(N, d_fixed, eta) for N in N_zoom]
        ax.plot(N_zoom, noisy_vals_zoom, '--', color=colors[i], alpha=0.7,
                label=r'N00N $\eta=%s$' % eta)

# Ideal limits
ax.plot(N_zoom, multipixel_a[2:], ':', color='red', lw=1.5,
        label=r'Optimal $|\psi_s\rangle$ (Ideal)')
ax.plot(N_zoom, noon_a[2:], '--', color='green', lw=1.5,
        label='N00N States (Ideal)')
...
for i, eta in enumerate(etas_to_test):
    if valid_d[eta]:
        ...
        # Noisy N00N (Dashed, same color)
        # N = d+1 for n=1
        noisy_vals = [noisy_noon(d+1, d, eta) for d in valid_d[eta]]
        ax.plot(valid_d[eta], noisy_vals, '--', color=colors[i], alpha=0.7,
                label=r'N00N $\eta=%s$' % eta)

ax.plot(d_values_b, multipixel_b, ':', color='red', lw=1.5,
        label=r'Optimal $|\psi_s\rangle$ (Ideal)')
ax.plot(d_values_b, noon_b, '--', color='green', lw=1.5,
        label='N00N States (Ideal)')
